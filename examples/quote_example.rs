...
use drfraudsworth_jupiter_adapter::{
    accounts::addresses::{CRIME_MINT, FRAUD_MINT, NATIVE_MINT, PROFIT_MINT},
    known_instances, known_sol_pool_keys, SolPoolAmm, VaultAmm,
};
...
fn main() {
    ...
    println!(
        "   Output:  {} PROFIT (100:1 ratio)",
        vault_quote.out_amount
    );
    ...
    let reverse_quote = reverse_vault
        .quote(&QuoteParams {
    ...
    println!(
        "   Output:  {} FRAUD (1:100 ratio)",
        reverse_quote.out_amount
    );
    ...
    println!(
        "   SOL Pools ({} instances, created via from_keyed_account):",
        sol_keys.len()
    );
    for key in &sol_keys {
        println!("     - {}", key);
    ...
    println!(
        "   Vault Conversions ({} instances, pre-built):",
        vault_instances.len()
    );
    ...
    println!(
        "   Total: {} Amm instances covering 8 swap directions",
        sol_keys.len() + vault_instances.len()
    );
}
