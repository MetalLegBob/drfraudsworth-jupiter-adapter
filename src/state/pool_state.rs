...
//
// Extracts reserves, mint_a, and lp_fee_bps from on-chain PoolState
...
/// Parsed PoolState -- fields needed for Jupiter quoting.
...
impl ParsedPoolState {
    ...
    ///
    /// Validates minimum length (155 bytes). Does NOT validate discriminator
    /// because the SDK consumer (Jupiter) provides the account data after
    /// filtering by owner program.
...
mod tests {
    ...
    /// Build a mock PoolState byte array.
    fn mock_pool_state(
        mint_a: &Pubkey,
        reserve_a: u64,
        reserve_b: u64,
    ...
    fn parse_reversed_order_pool() {
        // mint_a != NATIVE_MINT, so reserves are reversed
        let some_mint = Pubkey::new_unique();
        let data = mock_pool_state(&some_mint, 500_000_000, 100_000_000, 100);
        let parsed = ParsedPoolState::from_bytes(&data).unwrap();

        assert_eq!(parsed.mint_a, some_mint);

        // sol_and_token_reserves swaps because mint_a != NATIVE_MINT
        let (sol, token) = parsed.sol_and_token_reserves();
        assert_eq!(sol, 100_000_000);  // reserve_b becomes SOL
        assert_eq!(token, 500_000_000); // reserve_a becomes token
    }
    ...
    fn lp_fee_parsed_correctly() {
        let data = mock_pool_state(&NATIVE_MINT, 1, 1, 50);
        let parsed = ParsedPoolState::from_bytes(&data).unwrap();
