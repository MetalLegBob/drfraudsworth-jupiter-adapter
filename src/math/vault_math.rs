...
pub fn compute_vault_output(
    ...
    let is_input_crime_or_fraud = *input_mint == CRIME_MINT || *input_mint == FRAUD_MINT;
    let is_output_profit = *output_mint == PROFIT_MINT;
    let is_input_profit = *input_mint == PROFIT_MINT;
    let is_output_crime_or_fraud = *output_mint == CRIME_MINT || *output_mint == FRAUD_MINT;
...
mod tests {
    ...
    fn fraud_to_crime_not_supported() {
        assert_eq!(compute_vault_output(&FRAUD_MINT, &CRIME_MINT, 1000), None);
    }

    #[test]
    fn same_mint_not_supported() {
        assert_eq!(compute_vault_output(&CRIME_MINT, &CRIME_MINT, 1000), None);
    }
    ...
    fn profit_to_crime_overflow() {
        ...
        assert_eq!(compute_vault_output(&PROFIT_MINT, &CRIME_MINT, large), None);
    }
